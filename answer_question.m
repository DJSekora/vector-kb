function [answers,total_costs,path_strings] = answer_question(vec1,relation_term,predicates,db,h3,index,word)
...
[answer_projection, offset, tail_weights, tail_inds]=find_tails_no_debug(nearest_head_inds,db,h3, ic, same_relation, head_vectors, vec1, word, heads);
if size(nearest_head_inds,1)>1
    nearest_head_inds2=nearest_head_inds(2:end);
else
    nearest_head_inds2=nearest_head_inds;
end
[answer_projection2, offset2, tail_weights2, tail_inds2]=find_tails_no_debug(nearest_head_inds2,db,h3, ic, same_relation, head_vectors, vec1, word, heads);
...
params.pos=true;
params.cutoff=.002;
[path_strings,total_costs,answers] = best_path3(vec1,output_vector,h3, db, predicates, word,params);
